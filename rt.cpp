...
public:
    ...
    Vec3 refract(const Vec3& normal, float refractiveIndex) const {
        ...
        if (sinT2 > 1) return Vec3(0, 0, 0); // Total internal reflection
...
public:
    ...
    Vec3 color;
    float reflectivity;
    float refractiveIndex; // Added for refraction
...
Vec3 calculateColor(const Ray& ray, const Scene& scene, const Vec3& lightPos, int depth = 0) {
    ...
    if (hitSphere) {
        ...
        Vec3 ambient = hitSphere->color * 0.1f; // Ambient light
        float diffuseFactor = std::max(0.0f, normal.dot(lightDir));
        Vec3 diffuse = hitSphere->color * diffuseFactor; // Diffuse color
        ...
        Vec3 specular = Vec3(1.0f, 1.0f, 1.0f) * specularFactor; // White highlight
    ...
    // Background color
    return Vec3(0.2f, 0.2f, 0.3f); // Dark blue background
}

void render(Scene& scene, int width, int height) {
    std::vector<Vec3> image(width * height);

    Vec3 lightPos(5, 5, -5); // Light source position

    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            // Generate rays
            float nx = (float)x / (width - 1) * 2 - 1; // Normalized x
            float ny = (float)y / (height - 1) * 2 - 1; // Normalized y
            Ray ray(Vec3(0, 0, 0), Vec3(nx, ny, -1)); // Camera at (0,0,0)
...
int main() {
    Scene scene;
    scene.addSphere(Sphere(Vec3(0, 0, -5), 1, Vec3(1, 0, 0), 0.5f)); // Red sphere
    scene.addSphere(Sphere(Vec3(2, 0, -5), 1, Vec3(0, 1, 0), 0.5f, 1.5f)); // Green sphere with refraction
    scene.addSphere(Sphere(Vec3(-2, 0, -5), 1, Vec3(0, 0, 1), 0.5f)); // Blue sphere
